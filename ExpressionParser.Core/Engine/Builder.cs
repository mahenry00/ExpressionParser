...
namespace ExpressionParser.Engine
{
	internal static class Builder
	{
		...
		internal static LambdaExpression BuildExpressionFor(Type parameterType, TokenList tokens, string parameterName)
		{
			var root = BuildTree(tokens);
			var parameterExpression = parameterName == null ? Expression.Parameter(parameterType) : Expression.Parameter(parameterType, parameterName);
			var body = root.BuildExpression(parameterExpression);
			return Expression.Lambda(body, parameterExpression);
		}
